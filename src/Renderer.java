...
public class Renderer extends Canvas {
	...
	public void drawBoard(Board b, Camera c, Matrix transform) {
		Matrix align = new Matrix().translationMatrix(62, -15, 70);
		...
		for(Piece p:b.getWhitePieces()) {
			//System.out.println(p.getClass().getName());
				...
				Matrix position = new Matrix().translationMatrix(-pos.getX() * Board.getTileSize(), 0, -pos.getY() * Board.getTileSize());
				Mesh m = ModelLoader.getModel(p.getClass().getName()).getMesh(c, Matrix.multiply(transform, position, align));
